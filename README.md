...
Logo and visual identity: [BRAND.md](BRAND.md).
...
The authenticated GraphQL refresh fetches repository topics (shown separately from hand-curated listing tags and searchable alongside them), SPDX license metadata and its permissions/conditions/limitations, plus the stars and last default-branch commit. It batches requests and retains the last snapshot for any repository GitHub cannot return; check `needsReview` after each run. License detection does not replace a manual check of the repository's actual license and any separately licensed files.
...
| `/tool/[slug]` | Tool overview, key GitHub facts, optional repository metadata and similar tools |
